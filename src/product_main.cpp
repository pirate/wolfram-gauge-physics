...
int main(int argc, char** argv) {
    try {
        ...
        for (int index = 1; index < argc; ++index) {
            ...
            if (argument == "--steps") steps = static_cast<uint32_t>(std::stoul(value));
        ...
        FiberBundleConnection initial(
            BaseGraph({0, 1, 2}, {{0, 1}, {1, 2}, {2, 0}}), fiber);
        initial.set_transport(2, 0, Permutation({1, 2, 3, 0}));
        const auto product = wgphysics::product::evolve_product(graph, 0, initial);
        ...
        output << "{\n  \"schema\":2,\n"
               << "  \"semantics\":\"raw engine evolution plus exact joint base/gauge quotient\",\n"
               << "  \"steps\":" << steps << ",\n"
        ...
        for (std::size_t index = 0; index < product.physical_states.size(); ++index) {
            ...
            output << "    {\"id\":" << index << ",\"base_vertices\":"
                   << state.base().vertices().size() << ",\"base_edges\":"
                   << state.base().edges().size() << ",\"gauge_signature\":";
        ...
        for (std::size_t index = 0; index < product.events.size(); ++index) {
            ...
            output << "    {\"engine_event\":" << event.engine_event
                   ...
                   << ",\"physical_output\":" << event.physical_output_state
                   << ",\"subdivision\":[" << event.subdivided_from << ','
                   << event.fresh_vertex << ',' << event.subdivided_to << ']'
