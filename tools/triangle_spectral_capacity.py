...
def capacity_certificate(group, charges):
    ...
    face_charges = []
    for face, path in zip(geometry.faces, geometry.paths):
    ...
    return {'flat_full_bundle_upper_edge': 12,
            ...
            'conclusion': 'On the supplied triangular tori, n_+(L_bundle - 12 I) <= sum_f q(H_f) = Q. The reaction and vacancy rules conserve this upper bound, not the actual mode count.'}
