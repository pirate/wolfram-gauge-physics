...
- **An exact bridge to statistical transport—and a limit on this rule family.** Microscopic
  ...
  factorization also proves autonomous one-cell sector dynamics for every admitted involution.
...
2. **Derive richer local dynamics.** Search beyond unary maps and the first Hurwitz exchange for
   ...
   The square-fiber involution census has an exact autonomous-sector obstruction: prioritize
   multi-loop observables and explicit cell/fiber-changing constructions over larger runs of
   the same closed charge dynamics.
